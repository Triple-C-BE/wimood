...
A Python daemon that periodically fetches product data (SKU, title, price, stock) from Wimood's XML API, optionally enriches it by scraping product pages (images, descriptions, specs), and syncs it to a Shopify store.
...
**Entry point:** `main.py` — initializes managers and runs pre-flight checks once at startup, then loops: fetch Wimood products → (optional: enrich via scraping) → sync to Shopify → sleep.
